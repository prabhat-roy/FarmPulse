...
spark = SparkSession.builder.appName("farmpulse-segment").getOrCreate()
df = spark.read.format("iceberg").load("farmpulse_mart.user_features")
...
preds.select("user_id","prediction").writeTo("farmpulse_mart.user_segments").using("iceberg").createOrReplace()
