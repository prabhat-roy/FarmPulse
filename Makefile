# FarmPulse â€” top-level Makefile
...
help:
	@echo "FarmPulse â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for FarmPulse via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get farmpulse-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=farmpulse-tools" --grpc-web
