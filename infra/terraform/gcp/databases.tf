resource "google_compute_global_address" "pg" {
  name          = "farmpulse-${var.environment}-pg-psa"
  purpose       = "VPC_PEERING"
...
resource "google_sql_user" "farmpulse" {
  name     = "farmpulse"
  instance = google_sql_database_instance.postgres.name
  password = random_password.pg.result
}

resource "google_sql_database" "farmpulse" {
  name     = "farmpulse"
  instance = google_sql_database_instance.postgres.name
}
...
resource "google_pubsub_topic" "audit" { name = "farmpulse-${var.environment}-audit" }
